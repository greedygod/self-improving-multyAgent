...
Use heartbeat to keep `{dataPath}/` organized without creating churn or losing data.
...
Store mutable run state only in `{dataPath}/heartbeat-state.md`.

## Start of Every Heartbeat

1. Ensure `{dataPath}/heartbeat-state.md` exists.
2. Write `last_heartbeat_started_at` immediately in ISO 8601.
3. Read the previous `last_reviewed_change_at`.
4. Scan `{dataPath}/` for files changed after that moment, excluding `heartbeat-state.md` itself.
