...
slug: self-improving
version: 1.2.16-tztest
changelog: "[非官方修改]：支持多agent安装时，取消硬编码的记忆路径实现彼此记忆独立，并通过配置管理记忆存储路径"
...
metadata: {"clawdbot":{"emoji":"🧠","requires":{"bins":[]},"os":["linux","darwin","win32"],"configPaths":["{baseDir}/../self-improving/.self-improving.json"],"configPaths.optional":["./AGENTS.md","./SOUL.md","./HEARTBEAT.md"]}}
---
...
**Path Configuration:** When this skill is invoked, it first reads `{baseDir}/../self-improving/.self-improving.json` to determine the `dataPath`. If this file does not exist or the `dataPath` is invalid/incomplete, setup is triggered automatically.

**Data Storage:** All memory data lives in `{dataPath}/` with tiered structure. The actual path is configured by the user during setup and stored in `.self-improving.json`.

```
{baseDir}/../self-improving/
├── .self-improving.json  # Config file: { "dataPath": "/path/to/data" }
└── {dataPath}/
...
| Step | Action |
|------|--------|
| 1 | Skill reads `{baseDir}/../self-improving/.self-improving.json` |
| 2 | Parse `dataPath` field |
| 3 | If `dataPath` is missing/invalid, trigger setup |
| 4 | All subsequent file operations use `{dataPath}/` |
...
- Stores preferences in local files (`{dataPath}/`)
- Maintains heartbeat state in `{dataPath}/heartbeat-state.md` when the workspace integrates heartbeat
- Reads its own memory files on activation

This skill NEVER:
- Accesses calendar, email, or contacts
- Makes network requests
- Reads files outside `{dataPath}/`
...
**Config file:** `{baseDir}/../self-improving/.self-improving.json`
**Data directory:** `{dataPath}/` (path specified in config)

Data stored in `{dataPath}/`:
